		...
		if facility:
			...
			if logs:
				...
				if latest_forecast and isinstance(latest_forecast.predictions_json, list):
					forecast_values = [row.get("p50") for row in latest_forecast.predictions_json if isinstance(row, dict)]
					if forecast_values:
						plt.plot(
...
class DeliveryAdmin(admin.ModelAdmin):
	...
	def change_view(self, request, object_id, form_url="", extra_context=None):
		extra_context = extra_context or {}
		delivery = Delivery.objects.select_related("depot").prefetch_related("items__facility").filter(pk=object_id).first()
...
class ModelRegistryAdmin(admin.ModelAdmin):
	...
	def promote_to_active(self, request, queryset):
		for model in queryset:
			ModelRegistry.objects.filter(model_type=model.model_type, facility=model.facility).exclude(id=model.id).update(
				is_active=False
			)
	...
	def rollback_to_previous(self, request, queryset):
		...
		for model in queryset:
			prev = (
				ModelRegistry.objects.filter(model_type=model.model_type, facility=model.facility, version__lt=model.version)
				.order_by("-version")
				.first()
			)
			if prev:
				ModelRegistry.objects.filter(model_type=model.model_type, facility=model.facility).update(is_active=False)
				prev.is_active = True
...
def admin_dashboard(request):
	...
	kpis = {
		...
		"route_cost_reduction": PlanningCycle.objects.exclude(cost_reduction_pct__isnull=True).aggregate(v=Avg("cost_reduction_pct"))["v"]
		or 0.0,
